...
/**
 * Ask the host process to restart. Resolves when the host acknowledges; the
 * process restarts shortly afterwards.
 * @returns settlement after the acknowledgement.
 */
export async function requestRestart(): Promise<void> {
  const response = await fetch(LAUNCHER_API.restart, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: '{}',
  })
  if (!response.ok) throw new Error('restart request failed (HTTP ' + String(response.status) + ')')
}
